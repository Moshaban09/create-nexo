...
### 📝 Documentation
- **Examples**: Added `Backend Integration` documentation and keywords.
- **Readme**: Updated `README.md` to reflect the latest architecture, removed deprecated features, and standardized on `npx`-only usage.
- **CLI Reference**: Updated `docs/cli.md` with current flags and standardized on `npx`.

### ⚙️ Changed
- **Logic**: Synchronized `fileTree.ts` dry-run logic with the actual scaffolding engine.

### 🐞 Fixed
- **CLI**: Added missing `--silent` flag to the `create` command definition.
...
- **Quality**: **Linting Cleanup** — Resolved unused variables and constant conditions in `prompts.ts` for a cleaner codebase.
